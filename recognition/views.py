...
#we will create utility functions
...
def create_dataset(username):
	...
	# step for face detection

	print("Facial Detector loading ...")
	...
	print("Video Stream Initializing ...")
	vs = VideoStream(src=0).start() #identifier

	#id is stored here along with the face for face recognition

	sampleNum = 0  #this will act as the counter for our dataset
	...
	while(True):
		...
		gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) #classifier works in grayscale

		faces = detector(gray_frame,0)  #detecting image and giving the coordinates
		...
		for face in faces:
			...
			sampleNum = sampleNum+1 #incrementing the counter

			
			if face is None:
				print("face is none")
				continue
			#to only save the face part
			...
			# @parameters : x,y is the initial point of rectangle
			# @parameters : x+width and y+height will be the new point
			# @parameters : rectangle colour
			# @parameters : rectangle thickness
			cv2.rectangle(frame,(x,y),(x+w,y+h),(0,255,0),1)

			cv2.waitKey(50) #for pausing

		#Creates a window with window name "Face" and with the image img
		cv2.imshow("Add Images",frame)
		#Before closing it we need to give a wait command, otherwise the open cv wont work
		# @parametrs : delay of 1 milisecond
		cv2.waitKey(1)
		if(sampleNum>300):
			break

	vs.stop()  #the videostream stops by this action
	cv2.destroyAllWindows()  #windows get destroyed
...
def update_attendance_in_db_in(present):
	today=datetime.date.today()  #gets the current time when attendance is marked
	time=datetime.datetime.now()  #gets the current time when attendance is marked
...
def update_attendance_in_db_out(present):
	today=datetime.date.today()  #date when the employee leaves
	time=datetime.datetime.now()  #time when the employee leaves
	for person in present:
		user=User.objects.get(username=person)
		if present[person]==True:
...
def mark_your_attendance(request):
	...
	while(True):
		...
		img_counter = 0  #counting the images, initializing to 0
		...
		while True:
			...
			if not ret:
				...
				break
			cv2.imshow("Attendance in", frame)
	...
	while(True):
		...
		while True:
			...
			if not ret:
				print("failed to grab frame")
				break
			cv2.imshow("Attendance out", frame)
